"""
Напишите к задаче из предыдущих семинаров классы исключения с выводом подробной информации.
...
умножения матриц с созданием нового экземпляра класса, а также просмотра созданных матрицы
    ...
    def __add__(self, other):
        if len(self._matr) != len(other._matr) or len(self._matr[0]) != len(other._matr[0]):
            ...
            return Matrix(
            [[self._matr[i][j] + other._matr[i][j] for j in range(len(self._matr[0]))] for i in range(len(self._matr))])
